...
agent-terminal spawn --name "$SESSION_NAME" vim "$FILE_PATH"

# 2. Wait for vim to be ready (shows filename or new file indicator)
FILENAME=$(basename "$FILE_PATH")
agent-terminal wait-for -s "$SESSION_NAME" "$FILENAME" -t 10000 || \
  agent-terminal wait-for -s "$SESSION_NAME" "VIM" -t 5000
...
if [ -n "$CONTENT" ]; then
  ...
  # Enter insert mode
  agent-terminal key -s "$SESSION_NAME" i
  
  # Type the content
  agent-terminal type -s "$SESSION_NAME" "$CONTENT"
  
  # Exit insert mode
  agent-terminal key -s "$SESSION_NAME" Escape
...
agent-terminal type -s "$SESSION_NAME" ":wq"
agent-terminal key -s "$SESSION_NAME" Enter

# 5. Wait briefly for vim to exit
sleep 0.5

# 6. Check if session is still alive (it shouldn't be)
if agent-terminal list-sessions 2>/dev/null | grep -q "$SESSION_NAME"; then
  echo "Warning: vim session still active, killing..."
  agent-terminal kill -s "$SESSION_NAME"
fi
